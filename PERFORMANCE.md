...
standard maxes at 1800 clients. We can conclude that there is not a significant performance hit putting information into the database when comparing standard and colocated.

- `run_script` : Colocated deployment offers a faster run_script client than standard deployment. We can 
infer colocated deployment is able to transfer information faster when processing data than standard deployment. 
This is likely because communication time is cut when using colocated deployment. There are also not as many requests sent using colocated deployment than standard which result in faster completion for colocated.

- `run_model` : Colocated deployment offers a faster run_model client than standard deployment. Like mentioned before,
...
unpack_tensor. However, standard shows larger outside points than colocated. 
...
- `unpack_tensor` : We notice that for both colocated and standard, unpack_tensor is much faster than put_tensor. However,
both deployment's performance similarly to each other with the difference being highlighted in the outside points.
...
- `poll_list` : Polling tensors from the database shows no large performance difference when comparing a C++ client and a Python client. However, there is a large performance difference when comparing the use of a file system over a database. The file system shows faster retrieval of tensors. 

- `get_list` : Retrieving tensors from the database shows a very small performance difference when comparing a C++ client and a Python client. The C++ client performs a hundredth of a second faster than the Python client. However, there is a large performance hit when working with the file system with high-end distribution values of the violin plots reaching to 2.50 / 1.20 seconds in comparison to the C++ Client: 0.10 seconds and Python Client: 0.18 seconds. We can infer that reading from a database is much faster than from the file system.
