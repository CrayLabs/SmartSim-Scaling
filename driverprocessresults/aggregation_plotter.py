...
def aggregation_plotter_standard(run_cfg_path):
    ...
    function_names = ['loop_time', 'get_list']
    languages = ['cpp']
    ...
    # plt.box(put_tensor_df['client_total'], put_tensor_df['time'])
        png_file = Path("results/aggregation-standard-scaling/stats") / os.path.basename(run_cfg_path) / f"{function_name}.png"
...
def aggregation_plotter_standard_py_fs(run_cfg_path):
    print("test")
    sys.exit()
    palette = sns.set_palette("colorblind", color_codes=True)
