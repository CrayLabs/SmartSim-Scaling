...
def throughput_plotter_colocated(run_cfg_path):
    ...
    # plt.box(put_tensor_df['client_total'], put_tensor_df['time'])
        png_file = Path("results/throughput-colocated-scaling/stats") / os.path.basename(run_cfg_path) / f"{function_name}.png"
...
def throughput_plotter_standard(run_cfg_path):
    ...
    for function_name in function_names:
        ...
        axs[0].set_ylabel(f'{function_name}\nTime (s)')
    # plt.box(put_tensor_df['client_total'], put_tensor_df['time'])
        png_file = Path("results/throughput-standard-scaling/stats") / os.path.basename(run_cfg_path) / f"{function_name}.png"
        plt.savefig(png_file)
        print(png_file)
    sys.exit()