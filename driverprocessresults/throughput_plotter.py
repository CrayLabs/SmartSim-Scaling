...
def throughput_plotter(run_cfg_path):
    ...
    # print("\ntest: ")
    # raise SystemExit()
    nnodes_all = json.loads(attributes_dict['client_nodes']) #just client nodes
    ...
    for backend, nnodes in tqdm(product(backends, nnodes_all), total=len(backends)*len(nnodes_all), desc="Product loop"):
        ...
        loop_iters = json.loads(attributes_dict['iterations'])
        DB_cpu = json.loads(attributes_dict['database_cpus'])[0]
        pin = "False"
        sizes.sort()
        # print("\ntest: " + nnodes)
        # raise SystemExit()
        ...
        for DB_node in tqdm(DB_nodes, leave=False, desc=f"{backend}-{nnodes}"):
            ...
            for size in tqdm(sizes, leave=False, desc=f"{DB_node} DB nodes"):
                ...
                #print(nnodes)
                if json.loads(attributes_dict['colocated']) == 1:
        ...
        for dark in tqdm([True, False], leave=False, desc="Plot style loop"):
            ...
            # sys.exit()
            if save:
                png_file = Path(run_cfg_path) / f"{label}-{nnodes}-{backend.lower()}_{plot_color}.png"
