...
def throughput_plotter(session_path):

    config = configparser.RawConfigParser()
    config.read(session_path + "/run.cfg")
    
    attributes_dict = dict(config.items('attributes'))
    
    palette = sns.set_palette("colorblind", color_codes=True)

    backends = attributes_dict[''] #add
    nnodes_all = str(int(attributes_dict['client_nodes']) + int(attributes_dict['database_nodes']))


    # Adapt to your setup
    base_path = f"../throughput-scaling-{backend.lower()}"

    DB_nodes = attributes_dict['database_nodes']
    sizes = attributes_dict['tensor_bytes']
    threads = attributes_dict['client_total']
    loop_iters = attributes_dict['iterations']
