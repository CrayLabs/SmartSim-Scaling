...
CLIENT_ALLOC = 280                     # number of nodes in client alloc
CLIENT_NODES = [280]                   # list of node sizes to run clients within client alloc
...
def setup_run(nodes, tasks):
    ...
    model.attach_generator_files(to_copy=[MODEL_DIR, "./process_results.py"])
    exp.generate(model)
    return model
...
def get_stats(data_locs):
    """Obtain inference statistics"""

    all_data = None
    for data_path, name, job_info in data_locs:
